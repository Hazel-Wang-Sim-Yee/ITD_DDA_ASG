...
Testing
Go to website to create a new account
Enter email and password for new account
Go to the "logInPage" page in Unity Application
Log in to the application using valid email and password
Enter name for bon
Choose bon type
Move camera to find a valid flat ground to spawn bon
Wait for bon's stats to decrease enough that user will want to increase their stats back to full
Use image detection to spawn food for bon
Feed bon
Use image detection to spawn shower for bon
Clean bon
Click UI button for sleeping to make the bon rest
Click UI button again when bon is fully rested to stop sleep
Click UI button for Activity
Play Activity game
Exit back to main game scene
Press log out button
Use Website to check Bon's stats
